...
namespace UnityTools
{
    public static class BC7Decoder
    {
        public static byte[] ReadBC7(byte[] data, int width, int height)
        {
            ...
            var m1Endpoints = new byte[2 * 2 * 3];
            var mxEndpoints = new byte[3 * 2 * 4];
            var subsetIndex = new byte[16];
            var anchorIndex = new byte[4];
            var colorIndex = new byte[16];
            var alphaIndex = new byte[16];

            var endpointStart = new byte[4];
            var endpointEnd = new byte[4];
