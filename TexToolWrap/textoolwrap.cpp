...
bool GetPVRTexLibModes(int mode, PVRTuint64& pvrtlMode, PVRTexLibVariableType& pvrtlVarType)
{
	switch (mode)
	{
		...
		case 10: pvrtlMode = PVRTLPF_BC1; break;
		case 12: pvrtlMode = PVRTLPF_BC3; break;
...
EXPORT unsigned int EncodeByPVRTexLib(void* data, void* outBuf, int mode, int level, int space, unsigned int width, unsigned int height)
{
	PVRTuint64 pvrtlMode;
	PVRTexLibVariableType pvrtlVarType;
...
EXPORT unsigned int EncodeByISPC(void* data, void* outBuf, int mode, int level, unsigned int width, unsigned int height)
{
	...
	if (mode == 24) //BC6H
	{
		...
		switch (level)
		{
			case 1: GetProfile_bc6h_basic(&bc6hsettings); break;
			case 2: GetProfile_bc6h_slow(&bc6hsettings); break;
			case 3: GetProfile_bc6h_veryslow(&bc6hsettings); break;
		}
	...
	else if (mode == 25) //BC7
	{
		...
		switch (level)
		{
			case 1: GetProfile_alpha_fast(&bc7settings); break;
			case 2: GetProfile_alpha_basic(&bc7settings); break;
			case 3: GetProfile_alpha_slow(&bc7settings); break;
		}
...
EXPORT unsigned int DecodeByCrunchUnity(void* data, unsigned int byteSize, void* outBuf, int mode, unsigned int width, unsigned int height)
{
...
EXPORT unsigned int EncodeByCrunchUnity(void* data, void* outBuf, int mode, int level, int space, unsigned int width, unsigned int height)
{
